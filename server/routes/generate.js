...
// System prompt from the Prompt Design Document (output structure is now enforced
// via the Anthropic tool-use schema below, not by asking the model to hand-write JSON)
...
const REVIEW_TOOL = {
  ...
  input_schema: {
    type: 'object',
    properties: {
      good_points: { type: 'string', description: '做得好的地方，多条用换行分隔，若无则为空字符串' },
      improve_points: { type: 'string', description: '待改进点，多条用换行分隔，若无则为空字符串' },
      next_time_reminder: { type: 'string', description: '下次练习注意事项，多条用换行分隔，若无则为空字符串' },
      confidence_level: { type: 'string', enum: ['高', '中', '低'] },
      note: { type: 'string', description: '说明信息不足或术语存疑等情况，若无异常则为空字符串' },
    },
